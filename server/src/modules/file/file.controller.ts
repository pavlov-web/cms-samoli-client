  ...
  async upload(
    ...
    if (file.mimetype.includes('image')) {
      ...
      for (const size of sizes) {
        const bufferResize = await this.fileService.resizeImage(
          file.buffer,
          size,
        );
        const bufferResizeWebP = await this.fileService.resizeImage(
          buffer,
          size,
        );
        ...
        saveArray.push(
          ...
          {
            originalname: `${file.originalname.split('.')[0]}_${size}.${
